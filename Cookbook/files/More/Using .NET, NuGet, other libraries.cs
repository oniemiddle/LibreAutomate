...
/// Many other libraries can be downloaded, for example from <google>NuGet<> or <google GitHub .NET libraries>GitHub<>. To install NuGet packages, use menu <b>Tools > NuGet<>.
///
/// When a NuGet package or some other library is installed, need to add its reference to scripts where you want to use it. You can do it in the <b>NuGet<> tool or in <b>Properties<>. Then you'll find one or more new namespaces in the `Ctrl+Space` list, and can use them.
