...
/// - In the <b>Find in lists<> field type `shell` (optionally, just to make the list smaller).
...
/// - Click <b>OK<>. It adds a comment line like `/*/ com Shell32 ...; /*/` in the script.
...
/// - In other scripts you can use the same .NET assembly (don't need to convert again). Either copy-paste the `/*/ com Shell32 ...; /*/`, or in the <b>Properties<> dialog click the <b>...<> button and select from submenu <b>Use converted<>. Or convert again, it does not harm.
/// - COM is an old technology. Some downloaded COM components may be 32-bit only. Script processes are 64-bit by default and can't use 32-bit dlls. To use 32-bit dlls, in the <b>Properties<> dialog select role `exeProgram` and check <b>bit32<>.
