...
/// The above code declares variable <i>i<> = 0. Then, while <i>i<> is less than 3, executes statements in the `{ }` block and increments <i>i<> (`i++`).
/// 
/// Several ways to insert <.k>for<> code quickly:
/// - Type text like `3t` (it means "repeat 3 times").
/// - Type `forl` and select <.x>forLoopSnippet<>.
