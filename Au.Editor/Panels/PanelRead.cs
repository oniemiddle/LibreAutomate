...
class PanelRead {
	...
	//timer _trackingFloatingReadWindow;
	...
	//	if (w != wmain) {
	//		if (w.IsMinimized) w.ShowNotMinimized();
	//		else if (w.IsMaximized && w.Screen == wmain.Screen) w.ActivateL(true);
	//	}
	//	if (w != wmain && _CanManageFloatingPanel()) {
	//		_MoveFloatingPanel();
	//		if (!w.IsOwnedBy(wmain, 0) && !w.IsTopmost && !w.ZorderIsAbove(wmain)) w.ZorderAbove(wmain);
	
	//		//place the floating panel at the right half of the editor panel rect
	//		void _MoveFloatingPanel() {
	//			RECT re = Panels.Editor.P.RectInScreen(), rr = w.Rect;
	//			re.left = re.CenterX;
	//			if (rr != re) w.MoveL(re);
	//		}
	
	//		if (_trackingFloatingReadWindow == null) {
	//			_trackingFloatingReadWindow = timer.every(200, _ => {
	//				if (_CanManageFloatingPanel()) {
	//					var f = Api.GetFocus();
	//					//print.it(f);
	//					if (f == Panels.Editor.ActiveDoc?.AaWnd) {
	//						_trackingFloatingReadWindow.Stop();
	//						_trackingFloatingReadWindow = null;
	//						p.Visible = false;
	//					} else if (Api.GetCapture().Is0) {
	//						_MoveFloatingPanel();
	//					}
	//				} else {
	//					_trackingFloatingReadWindow.Stop();
	...
	//			});
	//		}
	//	} else if (_trackingFloatingReadWindow != null) {
	//		_trackingFloatingReadWindow.Stop();
	//		_trackingFloatingReadWindow = null;
	//	}
	
	//	bool _CanManageFloatingPanel() {
	//		return p.Floating && !w.IsMaximized && !w.IsMinimized && w.Screen == wmain.Screen && w.ClientRectInScreen.Intersect(wmain.ClientRectInScreen);
	//	}
	//}
	...
	_WebBrowser _browser;
	
	public void OpenDocUrl(string uri) {
		if (_browser == null) {
			_browser = new(this);
			P.Children.Add(_browser);
		}
		_BeforeOpeningArticle();
		_browser.Open(uri);
	}
	
	internal class _WebBrowser : Grid {
		PanelRead _panel;
		WebBrowser _wb;
		wnd _ies;
#if DEBUG
		...
		public _WebBrowser(PanelRead panel) {
			...
			_wb.Navigated += (sender, e) => {
				...
				Dispatcher.InvokeAsync(_LoadCompleted2); //in some cases this event is after LoadCompleted
			};
			
			_wb.LoadCompleted += (_, e) => {
				//print.it("LoadCompleted", _wb.CanGoBack, _wb.CanGoForward, e.Uri);
				_ApplyBrowserZoom(VisualTreeHelper.GetDpi(_wb).PixelsPerDip);
#if DEBUG
...
#endif
				Dispatcher.InvokeAsync(_LoadCompleted2);
			};
			
			_wb.DpiChanged += (_, e) => { _ApplyBrowserZoom(e.NewDpi.PixelsPerDip); };
			...
			void _ApplyBrowserZoom(double zoom) {
				try {
					if (_wb.Document != null) _wb.InvokeScript("eval", $"document.body.style.zoom = {zoom};");
				}
				catch { }
			}
		}
		...
		internal void ShowContextMenu(int x, int y, int contextFlag, string selectedText) {
			...
			m["New script", disable: contextFlag != 2] = o => {
				var name = pathname.getNameNoExt(_wb.Source.AbsolutePath);
				App.Model.NewItem("Script.cs", null, name + ".cs", true, new(true, _SelectedText()));
			};
			m.Separator();
			m["Open in web browser"] = o => { _OpenInWebBrowser(); };
			m["Always in web browser..."] = o => { DOptions.AaShow(DOptions.EPage.Other); };
...
public class _WebBrowserBridge { //note: must be public
	private readonly PanelRead._WebBrowser _browser;
	
	internal _WebBrowserBridge(PanelRead._WebBrowser browser) {
		_browser = browser;
	}
	...
	public void ShowContextMenu(int x, int y, int contextFlag, string selectedText) {
		_browser.ShowContextMenu(x, y, contextFlag, selectedText);
	}
...
class DocsHttpServer : HttpServerSession {
	...
	static string _GetPreviewHtml() {
		//print.clear();
		var doc = Panels.Editor.ActiveDoc;
		string name = doc.EFile.DisplayName, code = doc.aaaText;
		
		if (_RecipeCodeToHtml == null) {
			AssemblyLoadContext.Default.LoadFromAssemblyPath(App.Model.NugetDirectoryBS + @"-\Markdig.dll");
			var asm = AssemblyLoadContext.Default.LoadFromAssemblyPath(App.Model.DllDirectoryBS + "AuDocsLib.dll");
